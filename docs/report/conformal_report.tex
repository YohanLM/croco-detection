...
\begin{equation}
  \lamhat \;=\; \inf\Bigl\{\lambda :
   \underbrace{\tfrac{n}{n+1}\hat{R}_n(\lambda) + \tfrac{B}{n+1}}_{\text{finite-sample correction } \tilde R(\lambda)}
   \;\le\; \alpha \Bigr\}.
  \label{eq:crc-rule}
\end{equation}
$B = \sup L_i$ is the loss upper bound, carried by \code{Risk.loss\_upper\_bound}
(all losses in this report have $B=1$, so the correction reduces to $\tfrac{1}{n+1}$).
The $+\tfrac{B}{n+1}$ term accounts for one extra worst-case test sample and
...
The calibration and test sets each comprise $n{=}800$ labeled frames drawn from
the croco-clip orthophoto dataset. The splits satisfy the following disjointness
conditions, which we verified explicitly with \code{scripts/evaluation/check\_disjointness.py}:

\begin{itemize}[nosep]
  \item The \textbf{calibration split} is entirely disjoint from the training set,
    the training validation set, and the test split. It is the only fully clean
    partition: $\lamhat$ calibrated on it is valid for deployment on data from the
    same distribution.
  \item The \textbf{test split} is disjoint from the training set but \emph{coincides
    with the training validation split} --- the $800$ images on which
    \code{best.pt} was selected by early stopping. The model did not train on
    these images, but its weights were \emph{chosen} for good performance on them.
\end{itemize}

\noindent\textbf{Implication.} Because $\lamhat$ is calibrated on the clean
calibration split, the CRC guarantee $\mathbb{E}[L(\lamhat)]\le\alpha$ is
mathematically valid for deployment on new, truly unseen data. However, the
\emph{reported} test metrics (raw risk, calibrated risk, fully-missed counts) are
slightly optimistic: \code{best.pt} was selected for good performance on those
exact images, so their empirical loss sits somewhat below what one would observe
on a fully independent sample. The bias affects the baseline (raw risk) and
the test risk equally, so the relative improvement from CRC is unaffected; only
the absolute reported values should be read with this caveat in mind. Generating
a fully clean test split via \code{scripts/evaluation/generate\_clean\_splits.py}
and re-running all experiments is noted as a priority in \S\ref{sec:summary}.

Each frame may contain zero or one ground-truth croco clip: the splits are
\emph{not} filtered to positive-only images, preserving the distribution of
data the system sees in deployment. Images whose label file is empty (no clip
present) contribute a per-image loss of $0$ under every loss function defined in
this report, since there is no GT box to miss; they do not tighten the risk bound
but they do appear in the empirical average, so the reported risks and floors are
directly meaningful for the deployed setting.

Inference runs at a fixed objectness threshold $T = 0.30$ (YOLO's standard
operating point, \code{imgsz=640}): any predicted box whose confidence score falls
below $T$ is discarded before the expansion function is applied. False positives
--- predicted boxes on frames with no GT clip, or extra boxes on frames with one
clip --- are not penalized by the coverage loss (which measures GT coverage only),
but they inflate the prediction-set area; the efficiency metric and the per-run
false-positive diagnostics (\code{extra\_metrics} in \code{EvaluationResult})
track them separately. The SeqCRC top-1 selector (\S\ref{sec:seqcrc}) was
introduced precisely to suppress the surge of false-positive boxes that would
otherwise appear when Phase~1 lowers $T$ toward the floor.

\subsection{Terminology: raw risk and fully missed}

\paragraph{Raw risk.} The \emph{raw risk} is the empirical risk evaluated at the
detector's unmodified operating point: $\lambda = 0$ for geometric expansions
(the identity transformation, no box growth) or at the standard threshold
$T = 0.30$ for the confidence-filter knob. It quantifies how often the
unaugmented model already fails the loss criterion and serves as the
``without conformal calibration'' baseline in all tables.

\paragraph{Fully missed.} A frame is \emph{fully missed} if its per-image loss
is locked at $1.0$ regardless of how large $\lambda$ grows. Two distinct
mechanisms cause this:
\begin{itemize}[nosep]
  \item \emph{Empty prediction set} ($62/800$ at $T{=}0.30$, pixel-wise loss).
    The YOLO detector fires no prediction above $T$ on a frame that contains at
    least one GT clip. Geometric expansion cannot help: an empty set stays empty
    at any $\lambda$.
  \item \emph{Poor localisation} ($94{-}62 = 32$ additional frames, visible
    under the 75\%-coverage loss). The detector does fire, but no predicted box
    overlaps the GT at the $75\%$ coverage bar at $\lambda = 0$. This floor is
    attackable by box-growth expansion, which is why the geometric single-knob
    methods reduce the fully-missed count from 94 to $\sim$62 while the
    confidence-knob method does not.
\end{itemize}
Together these two sub-populations define the hard floor that any single-knob
method leaves unresolved. Only SeqCRC, which composes both mechanisms, can push
below that floor.
...
% ---------------------------------------------------------------------
\subsection{YOLO base detector performance}
\label{sec:yolo-baseline}
% ---------------------------------------------------------------------

\paragraph{Dataset composition.}
The 800-image test split contains \textbf{235 positive frames} (at least one
croco clip present) and \textbf{565 negative frames} (background only). The
splits are not filtered to positive-only images; the 29\% positive rate
reflects the deployment setting where background frames are the majority.

\paragraph{Box-level metrics (training validation split).}
Table~\ref{tab:yolo-box} reports the standard Ultralytics \code{val} metrics
produced during training. These are evaluated on the \emph{training validation
split} — the same $800$ images used to select \code{best.pt} by early stopping,
which as noted in \S4.1 coincides with the CRC test split. The numbers are
therefore optimistic by the model-selection bias described there.

\begin{table}[h]\centering
\caption{YOLO11n box-level detection metrics at $T{=}0.30$, \code{imgsz=640},
IoU threshold $0.50$ (Ultralytics default). Evaluated on the training val split
(epoch~22, best checkpoint). Due to the model-selection overlap with the CRC test
split (\S4.1), these numbers are slightly optimistic.}
\label{tab:yolo-box}
\begin{tabular}{lcccc}
\toprule
Class & Precision & Recall & mAP@.50 & mAP@.50:.95 \\
\midrule
croco clip & 0.714 & 0.733 & 0.777 & 0.273 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Image-level classification metrics (CRC test split).}
Table~\ref{tab:yolo-image} reports image-level TP/FP/FN/TN counts under two
criteria: (a) IoU~$\ge 0.10$ with the GT box (lenient detection criterion),
and (b) $\ge 75\%$ pixel coverage of the GT box (the same criterion Phase~2 of
SeqCRC optimises). The coverage criterion is stricter: a box that touches the
clip but covers less than $75\%$ of its area counts as FN. The two criteria
therefore decompose the failure modes — missed detections vs.\ poor localisation.
Results for the calibrated SeqCRC pipeline are included for direct comparison.

\begin{table}[h]\centering
\caption{Image-level classification on the 800-image test split (235 positive,
565 negative). IoU criterion: any predicted box overlaps GT at IoU~$\ge 0.10$.
Coverage criterion: the union of predicted boxes covers $\ge 75\%$ of the GT
pixel area (identical to the Phase-2 CRC loss). SeqCRC uses
$T_{\text{eff}}{=}0.099$ and $\lambda_{\text{loc}}{=}0.44$\,px.}
\label{tab:yolo-image}
\begin{tabular}{llccccccc}
\toprule
Criterion & Pipeline & TP & FN & FP & TN & Precision & Recall & F1 \\
\midrule
\multirow{2}{*}{IoU $\ge 0.10$}
  & Raw YOLO ($T{=}0.30$) & 173 & 62 &  0 & 565 & 1.000 & 0.736 & 0.848 \\
  & SeqCRC (calibrated)   & 220 & 15 &  2 & 563 & 0.991 & 0.936 & 0.963 \\
\midrule
\multirow{2}{*}{Coverage $\ge 75\%$}
  & Raw YOLO ($T{=}0.30$) & 141 & 94 &  0 & 565 & 1.000 & 0.600 & 0.750 \\
  & SeqCRC (calibrated)   & 204 & 31 &  2 & 563 & 0.990 & 0.868 & 0.925 \\
\bottomrule
\end{tabular}
\end{table}

Under the coverage criterion, the FN gap (94 $\to$ 31) is larger than under IoU
(62 $\to$ 15) because SeqCRC's Phase~2 expansion directly increases coverage
ratios: the 32 raw-YOLO frames where a box fired but covered less than $75\%$
of the clip are mostly recovered by the $+0.44$\,px additive margin.
Mean GT coverage on positive images rises from $0.644$ (raw YOLO) to $0.870$
(SeqCRC), confirming that the expansion closes genuine localisation slack rather
than merely growing boxes arbitrarily.

\begin{figure}[h]\centering
  \includegraphics[width=0.98\textwidth]{fig_raw_vs_seqcrc.png}
  \caption{Raw YOLO vs.\ calibrated SeqCRC on the 800-image test split
  (75\%-coverage criterion). Left: image counts per outcome category.
  Right: recall, precision, F1, and mean GT coverage for positive images.
  SeqCRC recovers $94-31=63$ additional clips at the cost of 2 false alarms.}
\end{figure}
...
\begin{itemize}[nosep]
  \item \textbf{Clean evaluation split (immediate priority).} As documented in
    \S4.1, the current test split coincides with the YOLO training validation split.
    Run \code{scripts/evaluation/generate\_clean\_splits.py} to re-shuffle the
    combined 1\,600-image pool into a fresh 800/800 partition, then re-run all CRC
    experiments. The calibrated $\lamhat$ is already valid; only the reported test
    metrics need refreshing.
