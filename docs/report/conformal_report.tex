...
the croco-clip orthophoto dataset. Both splits are entirely disjoint from the
images used to train the YOLO detector and from each other; using training images
in calibration would invalidate the exchangeability assumption on which the CRC
guarantee rests. The three partitions (train / calibration / test) were fixed
before any conformal experiment was run and are never re-randomised between runs.
...
% ---------------------------------------------------------------------
Before examining conformal experiments it is useful to characterise the base
YOLO model independently, using standard object-detection metrics on the same
$800$-image test split at $T = 0.30$. These metrics describe box-level detection
quality under the conventional IoU criterion and complement the CRC risk, which
is an image-level coverage metric.

\begin{table}[h]\centering
\caption{YOLO base detector metrics at $T{=}0.30$, \code{imgsz=640}.
\textsuperscript{$\dagger$} Epoch-22 (best-checkpoint) metrics on the
\emph{training} validation split — the same split used by \texttt{yolo} to
select \texttt{best.pt}. \textsuperscript{$\ddagger$} To be filled by running
\texttt{yolo val} on the CRC test split (\texttt{data/splits/test.txt}) so the
numbers are directly comparable to the raw-risk figures in \S5.2--5.4.}
\begin{tabular}{lcccc}
\toprule
Evaluation set & Precision & Recall & mAP@.50 & mAP@.50:.95 \\
\midrule
Training val split\textsuperscript{$\dagger$} & 0.714 & 0.733 & 0.777 & 0.273 \\
CRC test split\textsuperscript{$\ddagger$}    & ---   & ---   & ---   & ---   \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{Disjointness check (to verify on the GPU machine).} The CRC
guarantee requires that the calibration and test splits are entirely disjoint
from the training data. Because \texttt{best.pt} is chosen by its performance
on the training val split, that split must also be excluded from both the CRC
calibration and CRC test sets. Before reporting the CRC-test-split column above,
confirm that no filename in \texttt{data/splits/\{calibration,test\}.txt}
appears in either the train or val lists of \texttt{dataset\_800.yaml}.

The raw risks already in \S\ref{sec:yolo-baseline}--\S\ref{sec:seqcrc} give an
image-level proxy for recall on the CRC test split: a raw risk of $0.1175$ under
the 75\%-coverage loss means $11.75\%$ of CRC-test frames lack adequate GT
coverage at $\lambda{=}0$, which upper-bounds the frame-level miss rate at
$T{=}0.30$. The standard mAP/precision/recall from \code{yolo val} will complete
the picture with box-level IoU-based metrics.
